...
void CTracker::Update(std::vector<Person> &person)
{
    ...
    if(tracks.size()==0)
    {
        ...
        {
            CTrack* tr=new CTrack(person[i].pixel,dt,Accel_noise_mag);
            tracks.push_back(tr);
        }
    }
    ...
    for(int i=0;i<tracks.size();i++)
    {
        ...
        {
            Point2d diff=(tracks[i]->prediction-person[j].pixel);
            //euclid distance
    ...
    if(not_assigned_detections.size()!=0)
    {
        ...
        {
            CTrack* tr=new CTrack(person[not_assigned_detections[i]].pixel,dt,Accel_noise_mag);
            tracks.push_back(tr);
        }
    }
    ...
    for(int i=0;i<assignment.size();i++)
    {
        ...
        if(assignment[i] >= 0 && assignment[i] < M) // If we have assigned detect, then update using its coordinates,
        {
            ...
            tracks[i]->prediction=tracks[i]->KF->Update(person[assignment[i]].pixel, 1);
        }
